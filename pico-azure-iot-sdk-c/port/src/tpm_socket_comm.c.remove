...
static int send_socket_bytes(TPM_SOCKET_INFO* socket_info, const unsigned char* cmd_val, size_t byte_len)
{
    ...
    while (send_amt > 0)
    {
        sent_bytes = tcpsocketconnection_send(socket_info->socket_conn, pIterator, (int)send_amt);
        if (sent_bytes <= 0)
        {
...
static int read_socket_bytes(TPM_SOCKET_INFO* socket_info)
{
    ...
    int data_len = tcpsocketconnection_receive(socket_info->socket_conn, read_data, MAX_DATA_RECV);
    if (data_len == -1)
    {
...
{
    tcpsocketconnection_close(socket_obj);
}
...
TPM_SOCKET_HANDLE tpm_socket_create(const char* address, unsigned short port)
{
    ...
    else
    {
        ...
        if ((result->socket_conn = tcpsocketconnection_create()) == NULL)
        {
        ...
        {
            if (tcpsocketconnection_connect(result->socket_conn, address, port) < 0)
            {
                LogError("Failure: connecting to tpm simulator.");
